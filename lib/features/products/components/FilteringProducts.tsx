...
export default function FilteringProducts({ children }: PropsWithChildren) {
  ...
  return (
    <aside
      className={`${isFilterAsideOpen ? "-translate-x-6" : "-translate-x-[125%]"} absolute bottom-0 left-6 top-0 z-10 flex h-fit w-60 flex-col gap-y-8 rounded-sm border bg-white p-6 transition-transform lg:sticky lg:top-28 lg:translate-x-0 lg:border-r-[0.5px] [&>*]:flex-shrink-0`}
    >
      <Button
